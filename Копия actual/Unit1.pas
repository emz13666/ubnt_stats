...
//  Modems.CommandText := 'select * from modems as t1 LEFT OUTER JOIN ptx as t2 ON t1.id_modem=t2.id_modem';
  ...
  UpdateThread.LocationUpdate := '\\10.70.0.51\diskw\УКиСС\Участок АСУ ГТК\programs\ubiquiti_stats\ubnt_stats';
  UpdateThread.Resume;
...
begin
  ...
  Query.Close;
   { if CheckBox1.Checked then
    ...
    begin
      Query.sql.Text := 'select date, time, signal_level, color, status, name, x,y from statss, modems where ';
...
begin
 ...
 if Button=mbRight then begin
    ShowPointPosition(valueindex);
    exit;
 end;
 if not flagWLANConnections then
    try Chart1.Hint := namesModems[ValueIndex]+IntToStr(Trunc(Chart1.Series[0].YValue[ValueIndex]))+' dBm '+
...
var
  ...
  fl_on: byte;
begin
  //включение/отключение wifi для ноутбука (10.70.42.251)
   ...
   if fl_on =1 then begin
     ...
     if SSHconn.Execute then
       if FindLineSubstringInList('UP',sshconn.answer)>-1 then begin
         ...
         Query_3.ExecSQL;
         Query_3.Close;
         ShowMessage('Wifi у точки доступа 10.70.42.251 (SMOTR_2) включен.')
       end else
         ShowMessage('Wifi у точки доступа 10.70.42.251 (SMOTR_2) не включен: '#13+SSHconn.Answer.Text)
     else
       ShowMessage('Не удалось включить Wifi у точки доступа 10.70.42.251 (SMOTR_2).');
  ...
  begin
     ...
     if SSHconn.Execute then
       if FindLineSubstringInList('UP',sshconn.answer)=-1 then begin
         ...
         Query_3.ExecSQL;
         Query_3.Close;
         ShowMessage('Wifi у точки доступа 10.70.42.251 (SMOTR_2) отключен.')
       end else
         ShowMessage('Wifi у точки доступа 10.70.42.251 (SMOTR_2) не отключен: '#13+SSHconn.Answer.Text)
     else
       ShowMessage('Не удалось отключить Wifi у точки доступа 10.70.42.251 (SMOTR_2).');
     btnWifi_on.Enabled := True;
     btnWifi_off.Enabled := true;
     Screen.Cursor := crDefault;
     ProgressBar1.Position := 0;
