...
uses
  ...
  Dialogs, StdCtrls, Buttons, RXCtrls, unit1, MapFail, ComCtrls, Spin;
...
begin
     ...
     if FileExists(ExtractFilePath(Application.ExeName)+'map.bmp') then
        frmShowMap.imgmap.Picture.LoadFromFile(ExtractFilePath(Application.ExeName)+'map.bmp');
     if FileExists('O:\ДИТ\АСУ ГТК\map.bmp') then  frmShowMap.imgmap.Picture.LoadFromFile('O:\ДИТ\АСУ ГТК\map.bmp');
     ...
     while not form1.Query.Eof do begin
           ...
           paintx:=round((form1.query.fieldByName('x').AsInteger-12213)*kx);
           // на плане карьера координатная ось у перевернута. Координаты идут снизу вверх
           painty:=round(631-((form1.query.fieldByName('y').AsInteger-3087)*ky));
