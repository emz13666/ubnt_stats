...
begin
  ...
  ToolTipsDBGrid1.Tag := 1;
  Modems.Close;
  Modems.SQL.Text := 'select * from modems as m LEFT JOIN ptx as p ON m.id_modem=p.id_modem LEFT join equipment eq on m.name = eq.name  where eq.equipment_type=5 or eq.equipment_type=6 order by m.name';
  try
    Modems.Open;
  except
    DBConnection.Close;
...
begin
  ...
  Query_2.SQL.Text :='Insert into tmp_stats select ss.id, ss.id_modem, ss.mac_ap, ss.signal_level, ss.date, ss.time, ss.status, '+
                'ss.num_sats, ss.x, ss.y from statss ss LEFT JOIN modems m ON ss.id_modem=m.id_modem LEFT join equipment eq on m.name = eq.name  where eq.equipment_type = '+IntToStr(eqtype)+' and '+
...
begin
 ...
 begin
   ...
   ShellExecute(0,nil,PChar(pathFZ+'filezilla.exe'),pchar('ftp://admin:modular@'+Modemsip_address_1.AsString),nil,SW_restore);
   sleep(400);
 end;
